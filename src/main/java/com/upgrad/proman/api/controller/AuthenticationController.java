...
import com.upgrad.proman.service.business.AuthentiationService;
...
public class AuthenticationController {
    ...
    @Autowired
    private AuthentiationService authentiationService;

    @PostMapping(path = "/auth/login", produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    public ResponseEntity<AuthorizedUserResponse> login(@RequestHeader("authorization") final String authorization) throws AuthenticationFailedException {
        ...
        UserAuthTokenEntity userAuthToken = authentiationService.authenticate(decodedArray[0], decodedArray[1]);
        UserEntity userEntity = userAuthToken.getUser();

        AuthorizedUserResponse authorizedUserResponse = new AuthorizedUserResponse().id(UUID.fromString(userEntity.getUuid()))
                .firstName(userEntity.getFirstName()).lastName(userEntity.getLastName())
                .emailAddress(userEntity.getEmail()).mobilePhone(userEntity.getMobilePhone())
                .lastLoginTime(userAuthToken.getLoginAt());
